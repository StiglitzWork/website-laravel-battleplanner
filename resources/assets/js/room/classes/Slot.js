    ...
    **************************/
    ...
    generateDom(isOwner){
        ...
        if (isOwner) {
            ...
            if (this.operator == null) {
                dom += `<img type="image" id="operatorSlot-${this.id}" data-id="${this.id}" src="/media/ops/empty.png" class="cursor-click op-icon operator-slot operator-border no-pointer" style="border-color: black" />`
            } else{
                dom += `<img type="image" draggable="true" ondragstart="app.engine.drag(event)" id="operatorSlot-${this.id}" data-id="${this.id}" src="${this.operator.icon}" class="cursor-click op-icon operator-slot operator-border" style="border-color: #${this.operator.colour}"/>`
            }
        }
        return dom;
    }
